...
class VQ_VAE(Model):
    def __init__(self, original_dim=1, kernel_size=(3, 3), stride=1, padding=True,
                 num_embeddings=128, intermediate_dim=[32, 64], latent=64, name='vq_vae', **kwargs):
        super(VQ_VAE, self).__init__(name='vq_vae', **kwargs)
        self.encoder = Encoder(kernel_size=kernel_size, intermediate_dim=intermediate_dim, stride=stride, padding=padding)
        self.vector_quantizer = VectorQuantizer(num_embeddings, latent)
        self.decoder = Decoder(original_dim, kernel_size=kernel_size, intermediate_dim=intermediate_dim, stride=stride, padding=padding)
