...
double SystemInformationController::getGpu() {
    QProcess process;
    int memUsed = -1;
    int memTotal = -1;
    process.start("nvidia-smi", QStringList() << "--query-gpu=memory.total,memory.used --format=csv,no-header,nounits");
    if(!process.waitForFinished()) {
        qDebug() << "Error: " << process.errorString();
    }

    QString output = process.readAll();
    QStringList mems = output.split(",");
    memTotal = mems[0].toInt();
    memUsed = mems[1].toInt();

    return static_cast<double>(memUsed)/memTotal * 100;
}

QString SystemInformationController::getGpuName() {
    QProcess process;
    process.start("nvidia-smi", QStringList() << "--query-gpu=gpu_name --format=csv,no-header");
    if(!process.waitForFinished()) {
        qDebug() << "Error: " << process.errorString();
    ...
    QString output = process.readAll();
    qDebug() << "GPU name:" << output;

    return output;
}

//Function for network ping
QString SystemInformationController::getGateway() {
    QProcess process;
    QString gateway;
...
double SystemInformationController::getNetworkPing() {
...
void SystemInformationController::updateNetworkPing() {
...
QString SystemInformationController::networkPingText() const{
